...
- **RTC state**: `cronLastMinute` (epoch-minutes) in RTC RAM. Central `rtcRamValid()`/`rtcRamSetValid()` (compat.h/pm.cpp) replaces per-module magic words — set in main after all inits, checked on deep sleep wake. Components outside diptych may keep their own RTC-RAM magic word.
