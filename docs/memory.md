...
| `CONFIG_SPIRAM_MALLOC_RESERVE_INTERNAL` | `98304` | Reserves 96 KB of internal DRAM for DMA-capable allocations (WiFi static RX, the SD-on-SPI read bounce, mbedTLS/lwIP) that all peak together at boot. **The number tracks the image, not any one feature.** A full build (WiFi + LCD + LoRa + SD + Reticulum) stalls mid-boot at 64 KB with nothing else changed — the 96 KB was first raised alongside the TinyUSB stack behind [`usb cdc`](usb-console.md) and read as that stack's cost, but that stack was the straw, not the load. Every further internal-only consumer spends from here, so a boot-time alloc failure means the consumer list grew, and this is the dial. |
