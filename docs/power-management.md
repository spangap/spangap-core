...
| `"nvs"` | NO_DEEP_SLEEP | after `set`/`unset` until `save` |
| `"boot"` | NO_DEEP_SLEEP | during `waitForTime()` until NTP syncs |
| `"cron"` | NO_DEEP_SLEEP | held by default; released when `cron_enable=1` AND `cron_0` exists |
...
The cron lock prevents accidental deep sleep: without `cron_enable=1` and at least one cron entry, the lock stays held and `MSG_SYS_SLEEP` is never broadcast. This means `usb down` alone only achieves light sleep — deep sleep requires an explicit cron configuration that can wake the device.
...
**Graceful shutdown** (`network down`): sets `wantDown` flag and waits for 30s of idle (no `networkActivity()` calls from rtsp, web, log, or CLI network clients). Consumer tasks call `networkActivity()` in their active loops. `network up` cancels a pending shutdown. `network down!` forces immediate teardown.

On actual shutdown: broadcasts `MSG_NETWORK_DOWN` to all tasks, then `ntpStop()` + `mdns_free()` before `esp_wifi_stop()` (so mDNS goodbye multicast can be sent while RF is up), then `esp_wifi_deinit()` to fully tear down WiFi driver tasks (saves ~5mA vs `esp_wifi_stop()` alone). On `network up`: `esp_wifi_init()` + `esp_wifi_start()` to reinitialize. WiFi state survives deep sleep via `RTC_DATA_ATTR`.
...
- `network down` — Graceful shutdown (waits for 30s idle)
- `network down!` — Immediate WiFi teardown
- `network up` — Bring WiFi up (cancels pending graceful shutdown)
...
- `main/ipc.cpp` — `pmInit()`, `pmLockCreate/Acquire/Release`, `pmPollUsb()`, `cliUsbDown()`/`cliUsbUp()`, `pm` command, deep sleep lock table
- `main/ipc.h` — `pm_lock_type_t` enum, `pmLockCreate/Acquire/Release` API, `deepSleepAllowed()`
- `main/network_task.cpp` — `esp_wifi_deinit()`/`esp_wifi_init()`, `esp_sleep_enable_wifi_wakeup()`, `"network"` deep sleep lock
