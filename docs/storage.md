...
**External storage files** (`/state/storage/<mode>/<key.path>.json`): runtime registry. The first-level subdir under `storage/` is the **mode**, the filename's stem is the dot-path **prefix**. Today only mode `external` is implemented: file is mounted into `cfgRoot` at boot and saved to its own file when its sub-tree dirties (instead of going through `root.json`). Drop a file in `data/factory_state/storage/external/` and `scanExternals` picks it up — no compile-time registration. `s.lxmf` (potentially large message history) is externalised this way so unrelated config changes don't rewrite it (and vice versa). NOTE: externalising only changes the *file*; the sub-tree is still fully resident in the in-RAM cJSON tree, so an external is not a substitute for a true out-of-tree store when the data can grow without bound. (The IANA→POSIX timezone map used to live here as `s.time.zones`, but precisely because of that residency it was moved *out* of storage entirely — it is now a plain user-state file at `<stateDir>/timezones.json`, parsed transiently by `ntpApplyTimezone()` and never held in `cfgRoot`. See the timezone note below.) **Runtime registration**: `storageNewTreeFile("s.foo.bar")` registers a prefix at runtime (e.g. lxmf at conversation creation — see [reticulous evictable_storage.md](../../reticulous/docs/plans/evictable_storage.md)), so a subtree that accretes — one chatty contact's messages — rewrites only its own small file instead of churning `root.json`. Registration is **RAM-only** (no caller-side fs I/O, so it's safe to call from `itsPoll`-serving tasks); the physical file is created on the next flush by `writeExternalFile` as soon as a key under the prefix dirties it. **Deletion**: passing the prefix (or an ancestor of it) to `storageDeleteTree` flags the external (`pendingDelete`) so the next flush `fs_remove`s the file and unregisters it — without this the orphaned `.json` would survive and resurrect the deleted data on the next boot. Future modes (e.g. `flash-only/`) can be added by handling more subdirs in `scanExternals()`.
...
**Config API**: `storageGetInt()`, `storageGetStr()`, `storageSet()`, `storageUnset()`, `storageExists()`, `storageDefault()`, `storageDefaultTree()`, `storageSetTree()` (cJSON node), `storageDeleteTree()` (delete a key/subtree; also drops a matching external file, see above), `storageNewTreeFile()` (register a runtime external prefix, see above), `storageSave()` (force immediate flush), `storageCopy()` (prefix copy with optional `onlyIfTargetKeyExists`), `storageForEach()`, `storageList()`, `storageArrayCount()` (count consecutive `.0.`, `.1.`, ... entries for numbered arrays like `s.web.map.`).
...
Each subscribing task has one PSRAM-backed aux inbox; a producer's `storageSet` blocks up to 10 ms trying to enqueue, then drops with a `notify drop: … → [<task>] (inbox full)` warn (the write still committed — only its notification was lost). The storage task subscribes to `""` for browser/DC sync, so it receives *every* change: a single ~1 Hz stats publish across many producers (rnsd + each RNS interface + auto + tcp peers + lora + gps) lands a burst of >100 notifications in one drain window. Its inbox depth is therefore the Kconfig knob **`CONFIG_SPANGAP_STORAGE_NOTIFY_INBOX`** (default 256), sized to absorb that worst-case burst; raise it if a new high-frequency producer reintroduces drops.
...
**Timezone map (not a storage blob).** The IANA→POSIX timezone map `timezones.json` is **platform-owned** but deliberately ships as a *loose* factory file — checked in at `spangap-core/data/factory_state/timezones.json` (root of the state store, not under `storage/`) and folded into every consumer's image by `spangap_create_factory_image`'s data merge. Living outside `storage/` means it is never registered as an external and never attached to `cfgRoot`, so the ~15 KB map stays out of RAM; `ntpApplyTimezone()` parses it transiently (cJSON parse → read one POSIX string → free) only on a timezone change. It is **not** fetched at build time (that would make builds non-deterministic); refresh it as a release step with spangap-core's `make timezones`, which runs `scripts/update-zones.py` to pull the latest map from a GitHub-hosted source (ETag-cached) and rewrite the checked-in file. At runtime the browser keeps it current by HTTPS PUT to `/state/timezones.json` (the existing `/state` WebDAV mapping) when GitHub's ETag — tracked in `s.ntp.zones_etag` — changes.
