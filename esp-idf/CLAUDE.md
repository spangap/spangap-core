...
- **`storage_change_msg_t`** widened to `cb(4) + key[128] + val[128]` (was `48`/`44`). Supports hierarchical keys like `lxmf.directory.<32hex>.<field>` without subscriber-notify truncation warnings. Required the `ITS_MAX_MSG_DATA` bump above.
