...
- `lcd-icons.py` — rasterize SVG launcher icons (depends on cairo +
  ...
  label-only tiles).
- `update-zones.py` — refresh `src/timezones_gen.c` (the built-in IANA →
  POSIX zone table: two strcmp-sorted rodata arrays behind
  `include/timezones.h`'s `tzLookup`). The result is committed here; every
  consumer compiles it in, so zone data rides each firmware update.
