...
def render(flash_mb: int, updater: bool,
           app_bytes: int, fixed_bytes: int, updater_bytes: int,
           state_floor: int = 0) -> str:
    # `container` = the FLASH size (CONFIG_ESPTOOLPY_FLASHSIZE): the bootloader
    # header size and the ceiling every partition must fit under. `state_floor`
    # = the builder's MAX FIRMWARE SIZE (CONFIG_SPANGAP_MAX_FIRMWARE_KB): the
    # offset where the runtime `state` partition begins, expressed here as the
    # top of a `reserved` partition so fs.cpp reads it back as end-of-table (no
    # baked scalar). Byte-granular and independent of the power-of-two container.
    ...
    if provisional:
        # Reserve a container-scaled slice for fixed; app gets the rest. Generous
        # on both so the first link + littlefs pack succeed; shrunk post-build.
        fixed_sz = align_up(container // FIXED_PROVISIONAL_DIV, DATA_ALIGN)
        app_sz = ((container - offset - fixed_sz) // APP_ALIGN) * APP_ALIGN
        if app_sz < APP_ALIGN:
            sys.exit(f"gen-partitions: container {flash_mb}MB too small for a provisional app")
    else:
    ...
    if end > container:
        sys.exit(f"gen-partitions: layout ({end:#x}) exceeds {flash_mb}MB flash "
                 f"container ({container:#x}) — raise CONFIG_ESPTOOLPY_FLASHSIZE or "
                 "trim app/fixed")
    ...
    # Reserved firmware-growth region up to the state floor (= max firmware size).
    # `state` (runtime) begins at the top of the on-flash table, so pushing the
    # table up to state_floor with an empty reserved partition fixes state's start
    # independent of the shrink-wrapped app/fixed sizes — and is where the build
    # ENFORCES the ceiling: firmware overflowing it is a hard error. Final pass
    # only; the provisional layout fills the container and has no real floor yet.
    if not provisional and state_floor:
        if state_floor > container:
            sys.exit(f"gen-partitions: state floor ({state_floor:#x}) exceeds the "
                     f"{flash_mb}MB flash container ({container:#x}) — raise "
                     "CONFIG_ESPTOOLPY_FLASHSIZE")
        if end > state_floor:
            sys.exit(f"gen-partitions: firmware (app+fixed ends at {end:#x}) exceeds "
                     f"the max firmware size ({state_floor:#x}) — trim app/fixed or "
                     "raise CONFIG_SPANGAP_MAX_FIRMWARE_KB")
        mstart = align_up(end, DATA_ALIGN)
        if state_floor > mstart:
            # Data subtype 0x40: an inert, unmounted reserve. fs.cpp never opens
            # it; it exists only to carry the table top up to the floor.
            rows.append(("reserved", "data", "0x40", mstart, state_floor - mstart, ""))

    floorpart = f", state floor {state_floor/1024/1024:.2f}MB" if (not provisional and state_floor) else ""
    out = [
